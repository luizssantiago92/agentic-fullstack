...
export async function install(options = {}) {
  ...
  if (!hubOk) {
    if (!options.force) {
      throw new Error(
        "Spec-Driven Harness not detected. Run first:\n" +
          "   npx @luizsantiago/agentic-harness install\n" +
          "Or re-run with --force to install layer skills only (doctor will still require harness).",
      );
    }
    log(
      "⚠️ Spec-Driven Harness not detected (--force). Layer skills only; run harness install for gates.",
    );
  } else {
  ...
  log("🔒 Installing extension gates...");
  ...
  for (const script of EXTENSION_SCRIPT_ASSETS) {
    ...
    log(`✅ ${script} → ${HARNESS_SCRIPTS_DIR}`);
  }
...
export async function doctor(options = {}) {
  ...
  const layerGatePath = path.join(
    cwd,
    HARNESS_SCRIPTS_DIR,
    "validate_layer_routing.py",
  );
  const layerGateOk = await pathExists(layerGatePath);
  log(`Layer routing gate: ${layerGateOk ? "✅ present" : "⚠️ missing"}`);
  if (!layerGateOk) {
