...
export async function doctor(options = {}) {
  ...
  for (const skill of SKILL_ASSETS) {
    const p = path.join(cwd, ".cursor/skills", skill);
    const ok = await pathExists(p);
    log(`Skill ${skill}: ${ok ? "✅" : "❌ missing"}`);
    if (!ok) {
      issues.push(`skill_missing:${skill}`);
    }
  }
  ...
  const gatesDir = path.join(cwd, ".specs/harness/scripts/validate_spec.py");
  const gatesOk = await pathExists(gatesDir);
