...
import { resolveLayerGatePath } from "./scripts-dir.js";
...
export function resolveLayerGateScript(cwd = process.cwd()) {
  return resolveLayerGatePath(
    cwd,
    LAYER_GATE_FILENAME,
    path.join(PACKAGE_ROOT, "gates", LAYER_GATE_FILENAME),
  );
}
...
export function validateLayers(opts = {}) {
  ...
  if (!fs.existsSync(script)) {
    throw new Error(
      "Layer routing gate not found. Run: npx @luizsantiago/fullstack-floor-map install",
    );
  }
