...
- Artifacts remain in English (Spec Seatbelt / engineering-baseline rule).
- Layer tasks must not share the same file in parallel (Seatbelt `validate_tasks.py` / `loop-plan`).
- When globs overlap, refine paths in this file or split tasks.
- Vertical **feature**, horizontal **tasks**: one layer (Floor) per task `Files`.
- On Execute you may load at most **one** specialist catalog skill for the same Floor (plus ≤2 references).
